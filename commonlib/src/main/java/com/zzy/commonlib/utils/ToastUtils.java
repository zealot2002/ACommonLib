...
public final class ToastUtils {
    ...
    private static void show(final int resId, final int duration) {
        try {
            CharSequence text = application.getResources().getText(resId);
            show(text, duration);
    ...
    private static void show(final CharSequence text, final int duration) {
        Log.e(TAG,"show 1 threadName:"+ Thread.currentThread().getName());
        HANDLER.post(new Runnable() {
            @SuppressLint("ShowToast")
            @Override
            public void run() {
                cancel();
                iToast = ToastFactory.makeToast(application, text, duration);
                ...
                setBg(tvMessage);
                Log.e(TAG,"show 2 threadName:"+ Thread.currentThread().getName());
                iToast.show();
            }
        });
    }
    ...
    private static void show(final View view, final int duration) {
        HANDLER.post(new Runnable() {
            ...
            public void run() {
                cancel();
                iToast = ToastFactory.newToast(AppUtils.getApp());
                iToast.setView(view);
    ...
    private static View getView(@LayoutRes final int layoutId) {
        LayoutInflater inflate =
                (LayoutInflater) application.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    static class ToastWithoutNotification extends AbsToast {
        ...
        private static final AppUtils.OnActivityDestroyedListener LISTENER =
        ...
        public void show() {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N_MR1) {
                ...
                AppUtils.getActivityLifecycle().addOnActivityDestroyedListener(topActivity, LISTENER);
            } else {
                mWM = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
            ...
            mParams.packageName = application.getPackageName();
